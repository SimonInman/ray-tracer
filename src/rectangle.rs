use crate::{Material, HitRecord, Ray, BoundingBox, Point3};
...
impl Rectangle {
    ...
    pub fn hit(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        ...
        return Some(HitRecord{
            p: ray.at(t),
            normal: crate::Vec3 { x: 0.0, y: 0.0, z: 1.0 },
            t,
            is_front_face: true,
    ...
    pub fn bounding_box(&self, _time0: f32, _time1: f32) -> Option<BoundingBox> {
        return Some(BoundingBox{
            minimum: Point3{x: self.x0, y: self.y0, z: self.z-EPSILON},
            maximum: Point3{x: self.x1, y: self.y1, z: self.z-EPSILON},
        });
    }
}